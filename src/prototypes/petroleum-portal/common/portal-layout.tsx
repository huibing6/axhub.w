    ...
    <Layout className="min-h-screen" style={{ background: t.colorBgLayout }}>
      <Header style={{
        ...
        <nav style={{ marginLeft: 32, display: 'flex', alignItems: 'center', gap: 8, lineHeight: 'normal' }}>
          ...
          <Button
            type={portalType === 'personnel' ? 'primary' : 'text'}
            size="small"
            onClick={() => window.location.hash = '#/personnel/add'}
          >
            关键人员管理
          </Button>
          <Button
            type={portalType === 'inspection' ? 'primary' : 'text'}
            size="small"
            onClick={() => window.location.hash = '#/inspection/manage'}
          >
            现场考察
          </Button>
        </nav>
