...
export default function MaintQuery() {
    ...
    { key: 'editTime', title: '编辑时间', width: 110, dataIndex: 'editTime' },
    {
      ...
      render: () => (
        <Space size={2}>
          ...
          <Typography.Link style={{ fontSize: 13, color: '#ff4d4f' }} onClick={() => setProgressOpen(true)}>进度查询</Typography.Link>
        </Space>
      ),
    },
  ];
  ...
  const progressColumns = [
    ...
    { key: 'unit', title: '受理单位', width: 100, dataIndex: 'unit', render: (val: string) => <Typography.Link style={{ color: '#ff4d4f' }}>{val}</Typography.Link> },
