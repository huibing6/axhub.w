...
export default function SpQueryProcess() {
  ...
  const spColumns = [
    ...
    { key: 'action', title: '操作', width: 80, align: 'center' as const, render: () => <Typography.Link style={{ color: '#ff4d4f' }}>查看</Typography.Link> },
  ];
  ...
  return (
    <div>
      ...
      <Table
        ...
        dataSource={filteredData} rowKey="seq" pagination={{ pageSize: 10, showSizeChanger: true, showTotal: (t) => `共 ${t} 条` }} bordered size="middle" scroll={{ x: 1600 }}
      />
    </div>
  );
}
