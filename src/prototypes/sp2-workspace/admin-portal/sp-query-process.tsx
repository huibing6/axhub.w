...
const spFlowStatusColors: Record<string, string> = {
  '正常': 'success', '暂停': 'warning', '取消': 'default', '冻结': 'error',
  '待提交': 'default', '待复核': 'processing', '已拒绝': 'error', '已通过': 'success',
};
...
export default function SpQueryProcess() {
  ...
  const spColumns = [
    ...
    { key: 'action', title: '操作', width: 80, align: 'center' as const, render: () => <Typography.Link style={{ color: '#ff4d4f' }} onClick={() => window.location.hash = '#/admin/sp-query-process-detail'}>查看</Typography.Link> },
  ];
  ...
  const catColumns = [
    ...
    { key: 'spStatus', title: '服务商状态', dataIndex: 'spStatus', width: 100, align: 'center' as const, render: (val: string) => val ? <Tag color={spFlowStatusColors[val]}>{val}</Tag> : '—' },
    { key: 'catStatus', title: '服务品类状态', dataIndex: 'catStatus', width: 110, align: 'center' as const, render: (val: string) => val ? <Tag color={spFlowStatusColors[val]}>{val}</Tag> : '—' },
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <Typography.Title level={4} style={{ margin: 0 }}>流程查询</Typography.Title>
      ...
      <Card size="small" variant="outlined" style={{ marginBottom: 16 }}>
        ...
        <Row>
          <Col><Space size={8}><Button type="primary" danger onClick={() => {}}>查询</Button><Button onClick={handleReset}>重置</Button></Space></Col>
        </Row>
      </Card>
      <Table
        columns={activeTab === 'sp' ? spColumns : catColumns}
