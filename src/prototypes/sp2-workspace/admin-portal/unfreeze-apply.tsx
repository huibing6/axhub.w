/**
 * @name 解冻申请
 * 对齐原版：双Tab（服务商解冻/服务品类解冻）+ 搜索折叠 + 更多操作
 */
...
const spColumns = [
  ...
  { key: 'flowStatus', title: '流程状态', width: 160, dataIndex: 'flowStatus', ellipsis: true },
...
export default function UnfreezeApply() {
  ...
  const spActionColumn = (_: any, record: any) => (
    <Space size={4}>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }} onClick={() => message.info('查看供应商')}>查看供应商</Button>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }} onClick={() => message.info('冻结历史')}>冻结历史</Button>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }}>解冻</Button>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }}>终止</Button>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }}>预警信息</Button>
    </Space>
  );

  const productActionColumn = (_: any, record: any) => (
    <Space size={4}>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }}>查看服务品类信息</Button>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }} onClick={() => message.info('冻结历史')}>冻结历史</Button>
  ...
  return (
    <div>
      <Typography.Title level={4} style={{ margin: '0 0 16px' }}>解冻申请</Typography.Title>
      <Tabs
        activeKey={activeTab}
