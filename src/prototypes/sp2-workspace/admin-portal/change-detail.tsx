      ...
      <Card size="small" variant="outlined" style={{ marginBottom: 16 }}>
        ...
        <div style={{ padding: '0 16px' }}>
          {timelineItems.map((item, i) => (
            <div key={i} style={{ display: 'flex', marginBottom: i < timelineItems.length - 1 ? 24 : 0 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: i === timelineItems.length - 1 ? '#faad14' : '#52c41a', marginTop: 6, marginRight: 12, flexShrink: 0 }} />
              <div>
                <div><Text strong>{item.status}</Text></div>
                <div><Text type="secondary">{item.time} {item.operator}</Text></div>
                {item.opinion && <div><Text type="secondary">{item.opinion}</Text></div>}
              </div>
            </div>
          ))}
        </div>
