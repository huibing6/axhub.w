/**
 * @name 冻结解冻查询
 * 对齐原版：双Tab（服务商冻结解冻查询/服务品类冻结解冻查询）+ 搜索折叠 + 更多操作
 */
...
const spSearchFields = [
  ...
  { key: 'dataSource', label: '数据来源', type: 'select' as const, options: ['请选择', '配码管理', '信息变更'] },
...
const categoryData = [
  { index: 1, categoryCode: 'S0101000', categoryName: '咨询', spName: '大庆拓邦龙达科技有限公司', spType: '贸易商', mgmtType: '所属企业管理', workType: '暂停服务品类交易权限', applyType: '暂停服务品类交易权限', freezeReason: '资质到期', flowStatus: '待复核', catalogLevel: '二级' },
  { index: 2, categoryCode: 'S0201000', categoryName: '物化探服务', spName: '四川伯尔塔石油科技有限…', spType: '代理商', mgmtType: '所属企业管理', workType: '', applyType: '暂停服务品类交易权限', freezeReason: '其它', flowStatus: '已完成', catalogLevel: '一级' },
];
...
export default function FreezeQuery() {
  ...
  const spActionColumn = (_: any, record: any) => (
    <Space size={4}>
      ...
      <Button type="link" size="small" style={{ color: '#ff4d4f' }}>查看服务商</Button>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }} onClick={() => message.info('冻结历史')}>冻结历史</Button>
      <Button type="link" size="small" style={{ color: '#ff4d4f' }}>进度查询</Button>
    </Space>
  );
  ...
  return (
    <div>
      <Typography.Title level={4} style={{ margin: '0 0 16px' }}>冻结解冻查询</Typography.Title>
      <Tabs
        activeKey={activeTab}
              ...
              <>
                ...
                <Card size="small" variant="outlined" style={{ marginBottom: 16 }}>
                  <Row justify="space-between" align="middle">
                    <Col>
                      <Space>
                        <Button onClick={() => message.info('批量导出')}>批量导出</Button>
                      </Space>
                    </Col>
                    <Col>
                      <Button>表格定制</Button>
