...
import { theme, Typography, Card, Space, Input, Button, Table, DatePicker, message } from 'antd';
...
export default function RegReview() {
  ...
  const filteredData = rawData.filter(d => {
    ...
    if (codeFilter && !d.code.includes(codeFilter)) return false;
    return true;
  });
  ...
  const handleRowReview = (record: any) => {
    // 单行点击审核 → 进入详情页并定位到审批意见Tab
    window.location.hash = `#/admin/reg-detail?tab=approval&id=${record.idx}`;
  };

  const handleViewDetail = (record: any) => {
    window.location.hash = `#/admin/reg-detail?id=${record.idx}`;
  };
  ...
  const columns = [
    ...
    { key: 'submitter', title: '提交人', width: 90, dataIndex: 'submitter' },
    {
      ...
      render: (_: unknown, record: any) => (
        <Space size={4}>
          <Typography.Link style={{ fontSize: 13, color: '#ff4d4f' }} onClick={(e) => { e.stopPropagation(); handleViewDetail(record); }}>查看</Typography.Link>
          <Typography.Text type="secondary">|</Typography.Text>
          <Typography.Link style={{ fontSize: 13, color: '#ff4d4f' }} onClick={(e) => { e.stopPropagation(); handleRowReview(record); }}>审核</Typography.Link>
          <Typography.Text type="secondary">|</Typography.Text>
          <Typography.Link style={{ fontSize: 13, color: '#ff4d4f' }} onClick={(e) => { e.stopPropagation(); handleProgressQuery(record); }}>进度查询</Typography.Link>
        </Space>
      ),
    },
  ];
