...
const serviceDirColumns = [
  ...
  { key: 'action', title: '全部合并', width: 100, align: 'center' as const, render: () => <Typography.Link style={{ color: '#ff4d4f', fontSize: 13 }}>展开</Typography.Link> },
];
...
export default function RegDetail() {
  ...
  // 从 URL hash 参数中读取 tab，自动定位到指定页签
  useEffect(() => {
    ...
    if (queryIndex !== -1) {
      ...
      if (tab && ['basic', 'service', 'qualification', 'approval'].includes(tab)) {
        setActiveTab(tab);
      }
    }
  }, []);
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px 24px' }}>
              ...
              <FormField label="资质证明文件">
                <Typography.Link style={{ color: '#ff4d4f' }}>查看文件</Typography.Link>
              </FormField>
            </div>
          </div>
