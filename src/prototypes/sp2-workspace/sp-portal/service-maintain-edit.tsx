/**
 * @name 服务维护 - 编辑服务品类
 * 服务商工作台2.0 - 服务维护·编辑服务品类（改造自1.0，不再基于中标结果）
...
/* 列表回显数据源（与服务维护列表一致） */
const echoSource: { code: string; name: string; type: '专业' | '通用'; level: string }[] = [
  { code: 'S1001000', name: '租赁服务', type: '专业', level: '一级' },
  { code: 'S1001000', name: '租赁服务', type: '通用', level: '二级' },
  { code: 'S1001000', name: '租赁服务', type: '专业', level: '二级' },
];
...
export default function SpServiceMaintainEdit() {
  ...
  const echo = echoSource.find(s => s.code === code) || echoSource[0];

  const [services, setServices] = useState<ServiceItem[]>([
    { id: 1, code: echo.code, name: echo.name, type: echo.type, level: echo.level, expanded: true },
    { id: 2, code: 'S0401001', name: '仓储包装服务', type: '通用', level: '三级', expanded: false },
  ]);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        ...
        <Typography.Text type="secondary" style={{ fontSize: 14 }}>编辑服务品类</Typography.Text>
      </div>
              ...
              <Row gutter={24}>
                <Col span={12}>
                  <Form.Item label="资质等级" required>
                    <Select placeholder="请选择资质等级" defaultValue="a" options={[
                      { value: 'a', label: '甲级' },
                      { value: 'b', label: '乙级' },
                      { value: 'c', label: '丙级' },
                    ]} />
                ...
                <Col span={12}>
                  <Form.Item label="资质证书编号" required>
                    <Input placeholder="请输入资质证书编号" defaultValue="D113045678" />
                  </Form.Item>
                </Col>
                <Col span={12}>
                  <Form.Item label="信用评级">
                    <Select placeholder="请选择信用评级" defaultValue="aaa" options={[
                      { value: 'aaa', label: 'AAA' },
                      { value: 'aa', label: 'AA' },
                      { value: 'a', label: 'A' },
                      { value: 'bbb', label: 'BBB' },
