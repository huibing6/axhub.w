...
export default function FreezeQuery() {
  ...
  const expandedRowRender = (record: any) => {
    const isFreeze = record.disposalType === '冻结';
    const fields = [
      { label: '服务商编码', value: record.code },
      { label: '服务商名称', value: record.name },
      { label: '服务商管理类型', value: record.mgmtType },
      { label: '管理单位', value: record.adminUnit },
      { label: '处置类型', value: <Tag color={isFreeze ? 'orange' : 'green'}>{record.disposalType}</Tag> },
      { label: '流程状态', value: record.flowStatus },
      { label: '申请时间', value: record.applyTime },
      { label: '操作人', value: record.operator },
      { label: '处置原因', value: isFreeze ? '违反合同条款' : '合同已续签，解除冻结' },
      { label: '审批人', value: '管理员' },
      { label: '审批时间', value: record.applyTime },
      { label: '审批意见', value: record.flowStatus === '已驳回' ? '材料不齐全，请补充' : '同意' },
    ];
    return <DetailPanel title={`${record.disposalType}详情`} fields={fields} onClose={() => toggleRowExpand(record._key)} />;
  };
